...
// +kubebuilder:rbac:groups=envoy.kaasops.io,resources=accesslogconfigs/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=envoy.kaasops.io,resources=accesslogconfigs/finalizers,verbs=update

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
// TODO(user): Modify the Reconcile function to compare the state specified by
// the AccessLogConfig object against the actual cluster state, and then
...
func (r *AccessLogConfigReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&envoyv1alpha1.AccessLogConfig{}).
		Named("accesslogconfig").
		Complete(r)
}
