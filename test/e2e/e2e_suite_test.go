...
func TestE2E(t *testing.T) {
	...
	_, _ = fmt.Fprintf(GinkgoWriter, "Starting envoy-xds-controller integration test suite\n")
...
var _ = BeforeSuite(func() {
	...
	cmd = exec.Command("make", "docker-build", fmt.Sprintf("IMG=%s:%s", projectImage, projectImageTag))
	_, err = utils.Run(cmd)
	...
	Expect(utils.InstallEnvoyProxy()).To(Succeed(), "Failed to install envoy proxy")
